 ...
 * - 예: npm run prerelease:increment 2.19.0 dev (alpha)
 * - 예: npm run prerelease:increment 2.19.0 stage (beta)
 * 
 * 동작:
...
function main() {
  ...
  if (!baseVersion) {
    ...
    console.error('예: node increment-alpha-beta-version.js 2.19.0 dev (alpha)');
    console.error('예: node increment-alpha-beta-version.js 2.19.0 stage (beta)');
    process.exit(1);
  }
  
  // 베이스 버전 형식 검증
  if (!/^\d+\.\d+\.\d+$/.test(baseVersion)) {
    console.error('베이스 버전은 semver 형식이어야 합니다 (예: 2.19.0)');
    process.exit(1);
  }
