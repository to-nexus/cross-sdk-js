...
function useAppKitTheme() {
  ...
  const [themeMode, setInternalThemeMode] = reactExports.useState(modal$1.getThemeMode());
  const [themeVariables, setInternalThemeVariables] = reactExports.useState(modal$1.getThemeVariables());
  reactExports.useEffect(() => {
...
function useWalletInfo() {
  ...
  const walletInfo = reactExports.useSyncExternalStore(modal$1.subscribeWalletInfo, modal$1.getWalletInfo, modal$1.getWalletInfo);
  return { walletInfo };
}
function useAppKitState() {
  ...
  const [state, setState] = reactExports.useState(modal$1.getState());
  reactExports.useEffect(() => {
...
function useAppKitEvents() {
  ...
  const [event, setEvents] = reactExports.useState(modal$1.getEvent());
  reactExports.useEffect(() => {
...
function useAppKitWallet(parameters) {
  ...
  reactExports.useEffect(() => ChainController.subscribeKey("activeCaipAddress", (val) => {
    if (val) {
      WalletButtonController.setError(void 0);
      WalletButtonController.setPending(false);
    }
  }), []);
  reactExports.useEffect(() => ApiController.subscribeKey("walletButtons", (val) => {
    if (val.length) {
      WalletButtonController.setReady(true);
    }
  }), []);
  const handleSuccess = reactExports.useCallback((caipAddress) => {
  ...
  const connect = reactExports.useCallback(async (wallet) => {
    try {
