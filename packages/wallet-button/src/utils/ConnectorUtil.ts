  ...
  connectCrossExtensionWallet(): Promise<ParsedCaipAddress> {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        if (!crossWallet) {
          throw new Error('ONEpocket이 customWallets에 설정되지 않았습니다.')
        }
        if (!crossWallet.rdns) {
          throw new Error('ONEpocket RDNS가 설정되지 않았습니다.')
        }
        ...
        if (!crossWalletExtensionConnectors || crossWalletExtensionConnectors.length === 0) {
          throw new Error('ONEpocket 익스텐션이 설치되지 않았습니다.')
        }
        ...
        if (browserConnector) {
          ...
          throw new Error('ONEpocket 커넥터를 찾을 수 없습니다.')
        }
      } catch (err) {
        reject(err)
      }
