...
export const ConnectionController = {
  ...
  async connectExternal(options: ConnectExternalOptions, chain: ChainNamespace, setChain = true) {
    ...
    await client?.connectExternal?.(options)
  ...
  async etherSignMessage(params: { message: string; address: `0x${string}` }) {
    return this._getClient()?.etherSignMessage(params)
  },
  ...
  async signTypedDataV4(
    ...
    return this._getClient()?.signTypedDataV4(paramsData, customData, options)
  },
