...
export class Engine extends IEngine {
  ...
  public approve: IEngine['approve'] = async params => {
    ...
    try {
      ...
      configEvent.setError(EVENT_CLIENT_SESSION_ERRORS.session_approve_namespace_validation_failure)
      throw error
    }
  ...
  public approveSessionAuthenticate: IEngine['approveSessionAuthenticate'] =
    async sessionAuthenticateResponseParams => {
  ...
  private onSessionSettleRequest: EnginePrivate['onSessionSettleRequest'] = async (
    ...
    try {
      ...
      const { relay, controller, expiry, namespaces, sessionProperties, sessionConfig } =
        payload.params
      const pendingSession = [...this.pendingSessions.values()].find(s => s.sessionTopic === topic)
  ...
  private onSessionAuthenticateRequest: EnginePrivate['onSessionAuthenticateRequest'] =
    async args => {
      const { topic, payload, attestation, encryptedId, transportType } = args
      try {
  ...
  private isValidApprove: EnginePrivate['isValidApprove'] = async params => {
    if (!isValidParams(params)) {
      throw new Error(getInternalError('MISSING_OR_INVALID', `approve() params: ${params}`).message)
    }
