...
import { Logger, getDefaultLoggerOptions, pino } from '@walletconnect/logger'
import { SessionTypes } from '@walletconnect/types'
import { getSdkError, isValidArray, parseNamespaceKey } from '@walletconnect/utils'
import EventEmitter from 'events'

import { GENERIC_SUBPROVIDER_NAME, LOGGER, PROVIDER_EVENTS, RELAY_URL, STORAGE } from './constants'
...
export class UniversalProvider implements IUniversalProvider {
  ...
  public async request<T = unknown>(
    ...
    console.log('UniversalProvider request', args)
  ...
  public setDefaultChain(chain: string, rpcUrl?: string | undefined) {
    try {
      ...
      if (!(error as Error).message.includes('Please call connect')) {
        throw error
      }
    }
  }
