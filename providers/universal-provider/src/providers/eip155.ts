...
class Eip155Provider implements IProvider {
  ...
  public async request<T = unknown>(args: RequestParams): Promise<T> {
    console.log('universal provider request', args)
    switch (args.request.method) {
      case 'eth_requestAccounts':
        console.log('eth_requestAccounts', new Date().toLocaleTimeString())

        return this.getAccounts() as unknown as T
      case 'eth_accounts':
        console.log('eth_accounts', new Date().toLocaleTimeString())

        return this.getAccounts() as unknown as T
      case 'wallet_switchEthereumChain': {
        console.log('wallet_switchEthereumChain', new Date().toLocaleTimeString())

        return (await this.handleSwitchChain(args)) as unknown as T
      }
      case 'eth_chainId':
        console.log('eth_chainId', new Date().toLocaleTimeString())

        return parseInt(this.getDefaultChain()) as unknown as T
      case 'wallet_getCapabilities':
        console.log('wallet_getCapabilities', new Date().toLocaleTimeString())

        return (await this.getCapabilities(args)) as unknown as T
      case 'wallet_getCallsStatus':
        console.log('wallet_getCallsStatus', new Date().toLocaleTimeString())
    ...
    if (this.namespace.methods.includes(args.request.method)) {
      console.log('request', args.request.method, new Date().toLocaleTimeString())

      return await this.client.request(args as EngineTypes.RequestParams)
    }
    console.log('getHttpProvider', args.request.method, new Date().toLocaleTimeString())
