...
- **브랜치**: `release/*`
- **Sample Page**: alpha 버전 우선, 없으면 latest로 fallback
...
- **브랜치**: `release/*`
- **Sample Page**: beta 버전 우선, 없으면 latest로 fallback
...
Sample Page 빌드 시 각 환경에 맞는 패키지 버전을 자동으로 해결합니다:

```bash
# 스크립트 직접 실행
./scripts/resolve-sample-versions.sh <environment>
```

#### **버전 해결 전략**

1. **패키지 스캔**: `examples/` 디렉토리의 모든 `package.json`에서 `@to-nexus/` 패키지 자동 감지
2. **버전 해결**: 
   - **Dev**: `alpha` 태그 우선 → `latest` fallback
   - **Stage**: `beta` 태그 우선 → `latest` fallback  
   - **Prod**: `latest` 태그만 사용
3. **자동 업데이트**: dev/stage 환경에서만 `package.json` 자동 업데이트

#### **지원되는 패키지**

스크립트가 동적으로 감지하는 모든 `@to-nexus/*` 패키지:
- `@to-nexus/sdk`
- `@to-nexus/core`
- `@to-nexus/appkit`
- `@to-nexus/appkit-*`
- `@to-nexus/ui`
- `@to-nexus/common`
- 기타 모든 `@to-nexus/` 네임스페이스 패키지

#### **Docker 빌드 흐름**

```mermaid
graph TD
    A[Docker Build 시작] --> B{환경 확인}
    B -->|dev/stage| C[패키지 스캔]
    B -->|prod| D[Workspace 버전 사용]
    C --> E[버전 해결]
    E --> F{alpha/beta 존재?}
    F -->|Yes| G[해당 버전 사용]
    F -->|No| H[latest로 fallback]
    G --> I[package.json 업데이트]
    H --> I
    I --> J[의존성 설치]
    D --> J
    J --> K[빌드 실행]
